...
"remove the hardcoded SITES runtime source" Make sure that it keeps sites.ts as a seed source only (for migration) and does not delete it permanently — because if the database is left for any reason, the site will still run from the hardcoded data as a fallback.  
